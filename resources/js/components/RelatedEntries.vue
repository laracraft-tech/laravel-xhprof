    ...
    <div class="prof-data-list card mt-5" v-if="sortedProfData">
        <div>
            ...
            <table class="table table-hover table-sm mb-0">
                <thead>
                <tr>
                    <th v-for="(item, key) in profDataKey2Title" @click="sort(key)" :key="key" :class="key">
                        {{ item.partOne }}<br />
                        <span class="iconConnector">
                            {{ item.partTwo }}
                            <span v-if="profDataSortDir === 'desc'">
                                <svg :style="profDataSort !== key ? 'color:gray;' : ''" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="w-5 h-5">
                ...
                <tr v-for="data in sortedProfData">
                    <td v-for="(item, key) in profDataKey2Title" :key="key" :class="key">
                        {{data[key]}}
                    </td>
                    <td class="table-fit">
                        <router-link :to="{name:'request-preview', params:{id: data.id}}" class="control-action">

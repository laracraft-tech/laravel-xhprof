...
use LaracraftTech\LaravelSpyglass\FormatModel;
...
class RequestWatcher extends Watcher
{
    ...
    {
        $app['events']->listen(RequestStarted::class, [$this, 'startRequest']);
        $app['events']->listen(RequestHandled::class, [$this, 'recordRequest']);
    }

    public function startRequest(RequestStarted $event)
    {
        $this->xhprof->enable();
    }
    ...
    protected function payload($payload)
    {
        return $this->hideParameters($payload,
            Spyglass::$hiddenRequestParameters
        );
    }
        ...
        if (is_string($content)) {
            if (is_array(json_decode($content, true)) &&
                ...
                return $this->contentWithinLimits($content)
                        ? $this->hideParameters(json_decode($content, true), Spyglass::$hiddenResponseParameters)
                        : 'Purged By Spyglass';
            }

            if (Str::startsWith(strtolower($response->headers->get('Content-Type') ?? ''), 'text/plain')) {
                return $this->contentWithinLimits($content) ? $content : 'Purged By Spyglass';
            }
        }
